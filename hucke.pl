...
if ($output_opt){
   ...
   if($dir =~ /(.+)[\\|\/]$/) { $dir = $1 }; # remove a trailing \ or /
   my $cwd = getcwd;
...
foreach my $pkg_name (@packages){
   ...
   my $path;
   foreach my $lib_dir (File::Spec->catfile($libs_root, $pkg_name), $libs_ocaml) {
      my $_path = File::Spec->catfile($lib_dir,  $pkg)  ;
      if (-f $_path ) {
         die "Err: library '$pkg' occurs in in '$path' and in '$_path'" if $path;
         $path = $_path;
      }
   }
   if($path){
